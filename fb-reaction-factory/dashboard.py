...
def process_job(job_id, url, mode, rights_ok):
    try:
        if not rights_ok: raise RuntimeError("Confirm that you have rights or permission to reuse this source.")
        if mode not in VALID_MODES: raise RuntimeError("Choose Instagram Reaction or TV Mind USA Direct.")
        ...
        if mode == MODE_FB:
            ...
            result = {
                "mode": mode, "video_url": f"/source/{source.name}", "caption": "",
...
:root{--bg:#070a0f;--panel:#101722;--line:#263142;--text:#f6f8fb;--muted:#8d99aa;--accent:#6d5dfc;--good:#2dd881;--bad:#ff5d73}*{box-sizing:border-box}body{margin:0;background:#070a0f;color:var(--text);font-family:Arial,sans-serif}.wrap{max-width:900px;margin:auto;padding:36px 18px}.brand{font-size:28px;font-weight:900;margin-bottom:6px}.sub{color:var(--muted);margin-bottom:24px}.card{background:var(--panel);border:1px solid var(--line);border-radius:20px;padding:20px}.modes{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px}.mode{display:block;border:1px solid var(--line);border-radius:15px;padding:15px;background:#0a0f17;cursor:pointer}.mode.selected{border-color:var(--accent);box-shadow:inset 0 0 0 1px #6d5dfc55}.mode input{display:none}.mode strong{display:block}.mode span{display:block;color:var(--muted);font-size:12px;line-height:1.45;margin-top:5px}.badge{display:inline-block!important;width:auto;margin-top:9px!important;background:#171f2b;border-radius:999px;padding:5px 8px;font-weight:800}.row{display:flex;gap:10px}.url{flex:1;background:#070b11;border:1px solid var(--line);border-radius:13px;padding:15px;color:white;font-size:15px}button{border:0;border-radius:13px;background:var(--accent);color:white;padding:0 22px;font-weight:800;min-height:50px;cursor:pointer}.rights,.notice{margin-top:12px;font-size:13px;color:#c7cfdb}.notice{background:#0a0f17;border:1px solid var(--line);padding:11px;border-radius:12px;color:#aab4c2}.pipe{display:grid;grid-template-columns:repeat(4,1fr);gap:9px;margin-top:18px}.step{border:1px solid var(--line);border-radius:13px;padding:12px;background:#0a0f17;min-height:78px}.step small{display:block;color:var(--muted);margin-top:4px}.step.running{border-color:var(--accent)}.step.done{border-color:var(--good)}.step.skipped{opacity:.55;border-style:dashed}.step.error{border-color:var(--bad)}.status,.result{margin-top:14px;border:1px solid var(--line);background:#090e15;border-radius:13px;padding:14px}.track{height:7px;background:#18202c;border-radius:99px;margin-top:10px;overflow:hidden}.bar{height:100%;background:var(--accent);width:0}.error{display:none;color:#ffd4da;margin-top:9px}.result{display:none}.actions{display:flex;gap:8px;flex-wrap:wrap}.link{display:inline-block;padding:9px 12px;border-radius:10px;background:#1a2331;color:white;text-decoration:none}.caption{white-space:pre-wrap;font-size:12px;color:#cbd2dc;margin-top:12px;background:#070b11;padding:10px;border-radius:10px}.summary{color:#aab4c2;font-size:12px;margin-top:10px}@media(max-width:650px){.modes,.pipe{grid-template-columns:1fr 1fr}.row{flex-direction:column}button{min-height:48px}}
</style></head><body><div class="wrap"><div class="brand">Reaction Factory</div><div class="sub">Paste one URL and choose the workflow.</div><div class="card">
<div class="modes"><label id="m-ig" class="mode selected"><input type="radio" name="mode" value="instagram_reaction" checked><strong>Instagram Reaction</strong><span>Download → reaction edit → caption → Instagram.</span><span class="badge">REACTION ON</span></label><label id="m-fb" class="mode"><input type="radio" name="mode" value="facebook_direct"><strong>TV Mind USA Direct</strong><span>Download original → post straight to Facebook Page.</span><span class="badge">NO REACTION · NO EDIT</span></label></div>
<div class="row"><input id="url" class="url" placeholder="Paste Instagram / Facebook / Google Drive URL"><button id="start" onclick="startJob()">Start Instagram Reaction</button></div>
...
<div id="result" class="result"><div class="actions"><a id="video" class="link" target="_blank">Open video</a><a id="iglink" class="link" target="_blank" style="display:none">Open Instagram post</a></div><div id="summary" class="summary"></div><div id="caption" class="caption" style="display:none"></div></div>
</div></div><script>
const steps=['download','edit','caption','publish'];let timer=null;function mode(){return document.querySelector('input[name="mode"]:checked').value}function paint(n,s){document.getElementById('s-'+n).className='step '+(s||'waiting')}function modeUI(){const ig=mode()==='instagram_reaction';document.getElementById('m-ig').classList.toggle('selected',ig);document.getElementById('m-fb').classList.toggle('selected',!ig);document.getElementById('start').textContent=ig?'Start Instagram Reaction':'Post to TV Mind USA';document.getElementById('notice').innerHTML=ig?'<b>Instagram Reaction:</b> creates your reaction version and posts it to Instagram.':'<b>TV Mind USA Direct:</b> downloads the original and posts it as-is. No reaction or edit.';document.getElementById('e1').textContent=ig?'2 Reaction Edit':'2 No Edit';document.getElementById('e2').textContent=ig?'Build reaction':'Keep original';document.getElementById('c1').textContent=ig?'3 Caption':'3 Skip Caption';document.getElementById('c2').textContent=ig?'Title + tags':'No processing';document.getElementById('p2').textContent=ig?'Instagram':'TV Mind USA'}document.querySelectorAll('input[name="mode"]').forEach(x=>x.addEventListener('change',modeUI));modeUI();
async function startJob(){const url=document.getElementById('url').value.trim(),rights=document.getElementById('rights').checked;if(!url)return alert('Paste a URL first.');if(!rights)return alert('Confirm reuse rights first.');document.getElementById('start').disabled=true;document.getElementById('result').style.display='none';document.getElementById('error').style.display='none';const r=await fetch('/api/start',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({url,mode:mode(),rights_ok:rights})}),d=await r.json();if(!r.ok){document.getElementById('start').disabled=false;return alert(d.error||'Could not start')}poll();if(timer)clearInterval(timer);timer=setInterval(poll,1000)}
...
function show(r){if(!r)return;document.getElementById('result').style.display='block';document.getElementById('video').href=r.video_url||'#';const ig=document.getElementById('iglink');if(r.instagram_permalink){ig.href=r.instagram_permalink;ig.style.display='inline-block'}else ig.style.display='none';document.getElementById('summary').textContent=r.summary||'';const c=document.getElementById('caption');if(r.caption){c.textContent=r.caption;c.style.display='block'}else c.style.display='none'}poll();
</script></body></html>'''
...
def api_start():
    ...
    if mode not in VALID_MODES: return jsonify({"error": "Choose Instagram Reaction or TV Mind USA Direct."}), 400
    with JOB_LOCK:
        if CURRENT_JOB["state"] == "running": return jsonify({"error": "A video is already processing."}), 409
    job_id = uuid.uuid4().hex[:10]
    reset_job(job_id, mode)
    threading.Thread(target=process_job, args=(job_id, url, mode, rights_ok), daemon=True).start()
